/**
 * @FilePath     : /opencl/myHandWrite/conv1d/conv1d.c
 * @Description  :  
 * @Author       : Huo yang 935585440@qq.com
 * @Version      : 0.0.1
 * @LastEditors  : Huo yang 935585440@qq.com
 * @LastEditTime : 2024-05-29 21:41:59
 ...
 * @return        {*}
**/
_Bool CreateMemObjects(cl_context context, cl_mem memObjects[4], Conv1dData* input, Conv1dFilter* filter)
{
    ...
    memObjects[1] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * filter->in_channel * filter->out_channel * filter->col, filter->weight_data, NULL);
    memObjects[2] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * filter->out_channel, filter->bias_data, NULL);
...
int main(int argc, char** argv)
{
    ...
    // 创建OpenCL程序
    program = CreateProgram(context, device, "conv1d.cl");
    if (program == NULL)
    {
        Cleanup(context, commandQueue, program, kernel, memObjects);
        return 1;
    ...
    // 创建OpenCL内核
    kernel = clCreateKernel(program, "Conv1D", NULL);
    if (kernel == NULL)
    {
    ...
    Conv1dFilter fileter = {3, 2, 3, 2, weight, bias};
    if (!CreateMemObjects(context, memObjects, &input_wave, &fileter))
    {
        Cleanup(context, commandQueue, program, kernel, memObjects);
        return 1;
    }
    ...
    errNum |= clSetKernelArg(kernel, 3, sizeof(int), &input_wave.channel); // in_channel
    errNum |= clSetKernelArg(kernel, 4, sizeof(int), &input_wave.col); // in_col
    errNum |= clSetKernelArg(kernel, 5, sizeof(int), &fileter.col); // kernel_size
    errNum |= clSetKernelArg(kernel, 6, sizeof(int), &fileter.stride); // kernel_size
    errNum |= clSetKernelArg(kernel, 7, sizeof(cl_mem), &memObjects[3]);
    if (errNum != CL_SUCCESS)
    {
    ...
    size_t globalWorkSize[2] = { fileter.out_channel, (int)((input_wave.col - fileter.col) / fileter.stride) + 1 };
    ...
    Conv1dData out_wave = {fileter.out_channel, (int)((input_wave.col - fileter.col) / fileter.stride) + 1};
    out_wave.data = (float *)malloc(sizeof(float) * out_wave.channel * out_wave.col);
    // 计算结果拷贝回主机
    errNum = clEnqueueReadBuffer(commandQueue, memObjects[3], CL_TRUE, 0, out_wave.channel * out_wave.col * sizeof(float), out_wave.data, 0, NULL, NULL);
    if (errNum != CL_SUCCESS)
    {
