/**
 * @FilePath     : /opencl/myHandWrite/sinc/sinc.c
 * @Description  :  
 * @Author       : Huo yang 935585440@qq.com
 * @Version      : 0.0.1
 * @LastEditors  : Huo yang 935585440@qq.com
 * @LastEditTime : 2024-06-06 16:28:42
...
int main(int argc, char** argv)
{
    ...
    // 创建OpenCL程序
    program = CreateProgram(context, device, "sinc.cl");
    if (program == NULL)
    {
        Cleanup(context, commandQueue, program, kernel, memObjects);
        return 1;
    ...
    // 创建OpenCL内核
    kernel = clCreateKernel(program, "Sinc", NULL);
    if (kernel == NULL)
    {
    ...
    int count_num = COUNT_NUM;
    start = clock();
    // 设置内核参数
    errNum = clSetKernelArg(kernel, 0, sizeof(cl_mem), &memObjects[0]);
    errNum |= clSetKernelArg(kernel, 1, sizeof(int), &count_num);
